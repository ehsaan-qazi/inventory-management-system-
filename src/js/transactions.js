...
// Session storage keys for in-progress transaction state
const CUSTOMER_TXN_DRAFT_KEY = 'inProgressCustomerTransaction';

// Save transaction draft to sessionStorage
function saveTransactionDraft() {
  ...
  sessionStorage.setItem(CUSTOMER_TXN_DRAFT_KEY, JSON.stringify(draft));
}

// Restore transaction draft from sessionStorage
function restoreTransactionDraft() {
  try {
    const draftJson = sessionStorage.getItem(CUSTOMER_TXN_DRAFT_KEY);
    ...
    // Restore paid amount and notes
    if (draft.paidAmount) {
      document.getElementById('paidAmount').value = draft.paidAmount;
      calculateBalance();
    }
    if (draft.notes) {
    ...
    console.error('Error restoring transaction draft:', error);
    return false;
  }
}

// Clear transaction draft from sessionStorage
function clearTransactionDraft() {
  sessionStorage.removeItem(CUSTOMER_TXN_DRAFT_KEY);
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (restoreTransactionDraft()) {
    console.log('Restored in-progress transaction draft');
  }
...
// Setup auto-save for draft on input changes
function setupDraftAutoSave() {
  ...
  if (paidAmountInput) {
    paidAmountInput.addEventListener('input', () => {
      if (billItems.length > 0 || currentCustomer) {
...
function selectCustomerFromSuggestion(customerId, customerName) {
  ...
  // Save draft when customer is selected
  saveTransactionDraft();
}
...
function removeItem(index) {
  ...
  // Update draft after removing item
  if (billItems.length > 0 || currentCustomer) {
    saveTransactionDraft();
  } else {
    clearTransactionDraft();
...
function clearForm() {
  ...
  // Clear draft from sessionStorage (explicit cancel)
  clearTransactionDraft();
}
...
async function viewTransaction(id) {
  try {
    ...
    const totalWeight = txn.items.reduce((sum, item) => sum + item.weight_kg, 0);
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
          ...
          <tbody>
            ...
            <tr style="border-top: 2px solid #000; font-weight: bold; background: #f5f5f5;">
              ...
              <td style="text-align: right; padding: 8px;">${formatWeight(totalWeight)}</td>
              <td colspan="2"></td>
            </tr>
          </tbody>
        </table>
